...
export class Terminal extends Component {
    ...
    constructor() {
        ...
        this.child_directories = {
            root: ["books", "projects", "personal-documents", "skills", "languages"],
            books: [ " 😅 " ],
            skills: ["Front-end development", "React.js", "jQuery", "Ajax", "Express.js", "SQL", "Firebase","mongoDB","Node.js","Machine Learning","Data analytics"],
            projects: [ "Crop Analyzer","VQA(Visual question answering)","Image classification using CNN","React Project: Movie Website","React Project: Social Media"],
            interests: ["AI", "Deep Learning","Competition programming"],
        ...
        return (
            <React.Fragment key={id}>
                <div className="flex w-full h-5">
                    <div className="flex">
                        <div className=" text-black">{HostName} </div>
                        <div className="text-black mx-px font-medium">:</div>
                        <div className=" text-black">{this.current_directory}</div>
                        <div className="text-black mx-px font-medium mr-1">$</div>
                    </div>
                    <div id="cmd" onClick={this.focusCursor} className=" bg-transperent relative flex-1 overflow-hidden">
                        ...
                        <div id={`cursor-${id}`} className=" float-left mt-1 w-1.5 h-3.5 bg-black"></div>
    ...
    childDirectories = (parent) => {
        ...
        this.child_directories[parent].forEach(file => {
            files.push(
                `<span class="font-bold mr-2 text-black">${file}</span>`
            )
        });
        files.push(`</div>`)
        return files;
    ...
    render() {
        return (
            <div className="h-full w-full bg-white text-black text-sm font-bold" id="terminal-body">
                {
                    this.state.terminal
                }
            </div>
